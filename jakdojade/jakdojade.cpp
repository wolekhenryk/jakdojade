...
str find_city_name(const Vector<str> &board, const int i, const int j) {
    ...
    str city_name =
        board[i].substr(start_index + 1, end_index - start_index - 1);
    return city_name;
}
...
shift where_city(const Vector<str> &board, const int i, const int j) {
    ...
    for (const auto &[delta_i, delta_j] : shifts) {
        if (!inside_map(
                board.size(), board[0].length(), i + delta_i, j + delta_j))
            continue;
        if (isupper(board[i + delta_i][j + delta_j]))
...
void dijkstra(const Vector<Vector<pair>> &graph,
              const int start_city_id,
              const int end_city_id,
              const hash_map<int, str> &code_to_city,
    ...
    heap<pair, pair_comparator> pq(edges);
    ...
    if (query_type == 0)
        cout << distances[end_city_id] << endl;
    else {
...
int main() {
    ...
    for (int i = 0; i < height; i++) {
        ...
        for (int j = 0; j < row.length(); j++) {
            if (row[j] == '.' || isalpha(row[j]) || isalnum(row[j]))
                grid2d[i][j] = -2;
